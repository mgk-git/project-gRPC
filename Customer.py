    ...
    def executeEvents(self):
        ...
        for evnt in self.events :
            if evnt["interface"] == 'query':
                response = self.stub.MsgDelivery(service_pb2.RequestMsg(id=evnt["id"], client_type='customer',type='query'))
                self.output["recv"].append({"interface":"query","result":response.status_msg,"money":response.balance})
            elif evnt["interface"] == 'withdraw':
                response = self.stub.MsgDelivery(service_pb2.RequestMsg(id=evnt["id"],client_type='customer',type='withdraw',money=evnt["money"]))
                self.output["recv"].append({"interface":"withdraw","result":response.status_msg})
                print("withdraw")

            elif evnt["interface"] == 'deposit':
                response = self.stub.MsgDelivery(service_pb2.RequestMsg(id=evnt["id"],client_type='customer',type='deposit',money=evnt["money"]))
